                    ...
                    <div class="card-block">
                            ...
                            <div class="row justify-content-md-between">
                                <div class="col col-lg-7 col-xl-5 form-group">
                                    <div class="input-group">
                                        ...
                                        <a href="{{route('admin/luckyWheelHistory/luckyWheelHistory.index')}}"
                                           class="btn btn-warning ml-1"><i class="fa fa-refresh"></i></a>
                                    </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <table class="table table-hover" id="controlCandles">
                                ...
                                @if(isset($history) && ! $history->isEmpty())
                                    @foreach($history as $elm)
                                        <tr>
                                            <td class="text-center">{{$elm->user->email}}</td>
                                            <td class="text-center">{{$elm->note}}</td>
                                            <td class="text-center">
                                                {{date('Y-m-d H:i', strtotime($elm->created_at))}}
                                            </td>
                                        </tr>
                                    @endforeach
                                @else
                        ...
                        @if(isset($history) && ! $history->isEmpty())
                            <div style="margin: 0 auto; display: table"
                                 class="paginate text-center">{{$history->links()}}</div>
                        @endif
                    </div>
                </div>
            </div>
