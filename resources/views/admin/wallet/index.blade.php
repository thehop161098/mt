...
@section('body')
    ...
    <wallet-listing :data="{{ $data->toJson() }}" :url="'{{ url('admin/wallets') }}'" inline-template>

        <div class="row">
            <div class="col">
                <div class="card">
                    <div class="card-header">
                        <i class="fa fa-align-justify"></i> Wallet
                        <!-- <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                           href="{{ url('admin/wallets/create') }}" role="button"><i class="fa fa-plus"></i>&nbsp;
                            Add</a> -->
                            <button type="button" data-url="{{ url('admin/wallets/autoCreate') }}" class="btn btn-primary btn-spinner btn-sm pull-right m-b-0 autoCreate" title="autoCreate"><i
                        class="fa fa-plus"></i>&nbsp;Auto Create</button>
                    </div>
                    <div class="card-body" v-cloak>
                        <div class="card-block">
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover">
                                    ...
                                    <tr>
                                        ...
                                        <th is='sortable' :column="'coin'">Coin</th>
                                        <th is='sortable' :column="'code'">Code</th>
                                        <th is='sortable' :column="'private_key'">Private_key</th>
                                        <th is='sortable' :column="'amount_bsc'">Amount</th>
                                        <th is='sortable' :column="'user_id'">Email</th>
                                    ...
                                    <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                        <td class="bg-bulk-info d-table-cell text-center" colspan="7">
                                            <span class="align-middle font-weight-light text-dark">{{ trans('brackets/admin-ui::admin.listing.selected_items') }} @{{ clickedBulkItemsCount }}. <a
                                                    ...
                                                    @click="onBulkItemsClickedAll('/admin/wallets')"
                                            ...
                                            <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                        @click="bulkDelete('/admin/wallets/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                    ...
                                    <tr v-for="(item, index) in collection" :key="item.id"
                                        ...
                                        <td class="text-center">@{{ item.amount_bsc / 10 ** 18 }}</td>
                                        ...
                                        <td>
                                            <div class="row no-gutters">
                                                ...
                                                <form class="col" @submit.prevent="resetItem(item.resource_url + '/reset')">
                                                    <button type="submit" class="btn btn-sm btn-warning" title="Reset"><i
                                                ...
                                                <form class="col" @submit.prevent="transferItem(item.resource_url + '/autoTransfer')">
                                                    <button type="submit" class="btn btn-sm btn-danger"
                                                            title="Transfer"><i
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <button type="button" data-url="{{ url('admin/wallets/autoCreate') }}" class="btn btn-primary btn-spinner autoCreate" title="autoCreate"><i
                                    class="fa fa-plus"></i>&nbsp;Auto Create</button>
                            </div>
                        </div>
                    </div>
...
@section('bottom-scripts')
    <script src="{{asset('backend/js/wallet.js')}}"></script>
    @toastr_js
    @toastr_render
@endsection
